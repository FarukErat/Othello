...
void flipper(int col, int row, char turn)
{
    ...
    for (auto dir : dirs) // for each directions
    {
        ...
        {
            i = col - 1 + m * dir[0]; //multiplying by 'm'
            j = row - 1 + m * dir[1]; //to move along the directions

            if ((board[i][j] == empty) || (board[col - 1 + dir[0]][row - 1 + dir[1]] == turn))
            ...
            {
                k = col - 1 + dir[0]; //k and l are assigned the number along the legal direction
                l = row - 1 + dir[1];

                if (dir[0] == 0) // assigning the squares, when the direction is upward or downward
                {
                    while (l != j) //assign till j
                    {
                        board[k][l] = turn;
                        l += dir[1]; //incrase the number by the delta
                    }
                }

                if (dir[1] == 0) // assigning the squares, when the direction is left or right
                {
                    while (k != i) //assign till i
                    {
                        board[k][l] = turn;
                        k += dir[0]; //incrase the number by the delta
                    }
                }
                ...
                else  // assigning the squares, when the direction is cross
                {
                    while (k != i && l != j) //assign till i or j
                    {
                        ...
                        k += dir[0]; //incrase the numbers by the delta
                        l += dir[1];
                    }
                }
                break;
